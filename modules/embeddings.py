...
class Embeddings(nn.Module):
    def __init__(self,d_model, vocab,pad_id=0):
        super().__init__()   #Turning on the brain infrastructure of this class
        self.lut=nn.Embedding(num_embeddings=vocab,embedding_dim=d_model,padding_idx=pad_id) 
        #Lookup table for this class
        #Number of embeddings is the size of vocabulary
        #Dimension size of embedding matrix
        #Padding id is necessary as all sentences would not have the same number of words
        #so we dont treat empty spaces as tokens and waste computation on them
